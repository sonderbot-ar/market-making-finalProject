...
* In src/, created 3 files:
    1. avellaneda_stoikov.py - runs the mathematical model against the dataset (initialized inventory = 1 BTC) and outputs in src/results/
    2. trained_model.py - uses a trained machine learning model of XGBoost to do the same thing as avellaneda-stoikov and saves in src/results
    3. backtest_version1.py - loads the outputs from src/results, checks against original datasets and validates the outputs. It also calculates the final PnL, inventory variance, and trade counts at the end of the simulation.

Afternoon:
* Adjusted the input and output directories based on config.py for:
    a. avellaneda_stoikov.py
    b. trained_model.py
    c. backtest_version1.py
    d. kaggle_scrape.py
    e. data_preprocessor.py