...
def run_delta_neutral_backtest(merged_parquet_path, output_csv_path):
    ...
    # --- VPIN Calculation (Vectorized) ---
    print("Calculating Volume Clock and VPIN...")
    bucket_volume = 50.0  # The fixed volume V for each bucket (e.g., 50 BTC)
    vpin_window = 50      # Number of buckets n for the rolling average
    
    df['trade_qty'] = df['trade_qty'].fillna(0)
    df['is_buyer_maker'] = df['is_buyer_maker'].fillna(False)
    
    # Aggressive Sell (Buyer is Maker) vs Aggressive Buy (Seller is Maker)
    df['sell_vol'] = np.where(df['is_buyer_maker'] == True, df['trade_qty'], 0)
    df['buy_vol'] = np.where(df['is_buyer_maker'] == False, df['trade_qty'], 0)
    
    # Assign Volume Buckets
    df['cumulative_vol'] = df['trade_qty'].cumsum()
    df['bucket_id'] = (df['cumulative_vol'] // bucket_volume).astype(int)
    
    # Group by buckets to calculate imbalance
    bucket_df = df.groupby('bucket_id').agg(
        buy_vol_sum=('buy_vol', 'sum'),
        sell_vol_sum=('sell_vol', 'sum')
    ).reset_index()
    
    bucket_df['imbalance'] = abs(bucket_df['buy_vol_sum'] - bucket_df['sell_vol_sum'])
    bucket_df['vpin'] = bucket_df['imbalance'].rolling(window=vpin_window, min_periods=1).mean() / bucket_volume
    
    # Merge VPIN back to the main tick dataframe and forward-fill
    df = df.merge(bucket_df[['bucket_id', 'vpin']], on='bucket_id', how='left')
    df['vpin'] = df['vpin'].ffill().fillna(0)
    ...
    # Extract arrays for the high-frequency loop
    ...
    for i in range(N):
        ...
        exponent = np.clip(-gamma * wealth, -700, 700) 
...
run_delta_neutral_backtest("/Users/andresrodartee/market-making-finalProject/data/raw/btcusdt_master_merged.parquet", "delta_neutral_v2.csv")