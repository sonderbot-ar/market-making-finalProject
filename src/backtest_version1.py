...
def save_metrics(
    ...
    metrics = pd.DataFrame([{
        ...
        "fills": fills,
        "max_inventory_exposure": max_inventory_exposure,
        "final_pnl": final_pnl,
    ...
    metrics.to_csv(
        METRICS_DIR /
        f"{strategy_name.lower().replace(' ', '_')}_metrics.csv",
        index=False
    )
...
def main():
    ...
    run_backtest(
        file_path=RESULTS_DIR / "as_test.csv",
    ...
    run_backtest(
        file_path=RESULTS_DIR / "xgb_test_predictions.csv",
